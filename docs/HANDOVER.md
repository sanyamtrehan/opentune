...
Ear mode is complete and shippable. `mic` mode has its detector but no
microphone attached to it yet.
...
- `src/app/_components/` — headstock peg UI, tuning picker, tuning editor
- `src/app/_audio/` — the Web Audio shell around the synth
...
`mic` mode, and only `mic` mode:

1. An AudioWorklet that accumulates 128-sample quanta into a frame sized by
   `windowSizeFor` for the string being listened to, and calls `detectPitch`.
   Nothing but plumbing — the algorithm is done and tested.
2. `getUserMedia` with `echoCancellation`, `noiseSuppression` and
   `autoGainControl` all explicitly **off**. They default to on and all three
   mangle pitch content. This is the silent killer.
3. Needle UI, driven by cents from the target string.
4. Real guitar recordings for the test harness (see open question 5).
