  ...
  return (
    <header className="flex flex-none items-center justify-between gap-4 px-[clamp(1rem,4vw,1.75rem)] pt-3.5 pb-2">
      <div className="flex items-baseline gap-2.5">
        ...
        <span className="label-caps hidden sm:inline">every tuning, free</span>
      </div>
        ...
        {SECTIONS.map(({ label, ready }) =>
          ready ? (
            <span
              key={label}
              aria-current="page"
              className="rounded-full bg-accent-bg px-3.5 py-1.5 text-xs font-medium text-accent"
            >
              {label}
            </span>
          ) : (
            <button
              ...
              className="cursor-not-allowed rounded-full px-3 py-1.5 text-xs text-ink-ghost"
            >
              {label}
            </button>
          ),
